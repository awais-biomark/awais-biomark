...
const InvalidNamePasswordPopup = () => {
  ...
  return (
    <Grid container direction='row' justifyContent='center' alignItems='center'>
      <Box sx={{ boxShadow: '3', paddingBottom: '11px', px: '12px', paddingTop: '15px', borderRadius: '8px', backgroundColor: '#FFFFFF', maxWidth: '351px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <Box> <DoNotDisturbIcon sx={{ color: '#FFD75E', fontSize: 58 }} /></Box>
        <Box sx={{ textAlign: 'center', marginTop: '14px', marginBottom: '12px' }}>
          <Box style={titleStyle} className={classes.heading10}>Invalid Username/Password</Box></Box>
        <Box sx={{ textAlign: 'center', marginBottom: '11px' }}>
          <Box style={descStyle} className={classes.heading}>The entered information is wrong.</Box></Box>
        <Box sx={{ textAlign: 'center', marginBottom: '20px' }}>
          <Box style={descStyle} className={classes.heading}>Please try again.</Box></Box>
